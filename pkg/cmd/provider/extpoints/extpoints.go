...
var CloudProviders = &cloudProviderExt{
	newExtensionPoint(new(CloudProvider)),
}

type cloudProviderExt struct {
	*extensionPoint
}

func (ep *cloudProviderExt) Unregister(name string) bool {
	return ep.unregister(name)
}

func (ep *cloudProviderExt) Register(extension CloudProvider, name string) bool {
	return ep.register(extension, name)
}

func (ep *cloudProviderExt) Lookup(name string) CloudProvider {
	ext := ep.lookup(name)
	if ext == nil {
		return nil
	}
	return ext.(CloudProvider)
}

func (ep *cloudProviderExt) Select(names []string) []CloudProvider {
...
func (ep *cloudProviderExt) All() map[string]CloudProvider {
	all := make(map[string]CloudProvider)
	for k, v := range ep.all() {
		all[k] = v.(CloudProvider)
	}
	return all
}

func (ep *cloudProviderExt) Names() []string {
	var names []string
	for k := range ep.all() {
